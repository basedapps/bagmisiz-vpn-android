...
object V2rayConfigUtil {
  ...
  private fun inbounds(v2rayConfig: V2rayConfig): Boolean {
    try {
      val socksPort =
        Utils.parseInt(settingsStorage?.decodeString(AppConfig.PREF_SOCKS_PORT), AppConfig.PORT_SOCKS.toInt())
      val httpPort =
        Utils.parseInt(settingsStorage?.decodeString(AppConfig.PREF_HTTP_PORT), AppConfig.PORT_HTTP.toInt())
  ...
  private fun customLocalDns(v2rayConfig: V2rayConfig): Boolean {
    try {
      ...
      if (v2rayConfig.inbounds.none { e -> e.protocol == "dokodemo-door" && e.tag == "dns-in" }) {
        ...
        val localDnsPort =
          Utils.parseInt(settingsStorage?.decodeString(AppConfig.PREF_LOCAL_DNS_PORT), AppConfig.PORT_LOCAL_DNS.toInt())
