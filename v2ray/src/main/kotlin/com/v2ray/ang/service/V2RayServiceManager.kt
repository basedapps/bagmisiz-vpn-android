...
object V2RayServiceManager {
    ...
    override fun onReceive(ctx: Context?, intent: Intent?) {
      ...
      when (intent?.action) {
        Intent.ACTION_SCREEN_OFF -> {
          Log.d(ANG_LOGS_TAG, "SCREEN_OFF, stop querying stats")
          stopSpeedNotification()
        }

        Intent.ACTION_SCREEN_ON -> {
          Log.d(ANG_LOGS_TAG, "SCREEN_ON, start querying stats")
          startSpeedNotification()
        }
      }
    }
    ...
    GlobalScope.launch(Dispatchers.IO) {
      ...
      if (v2rayPoint.isRunning) {
        try {
          ...
          Log.d(ANG_LOGS_TAG, "measureV2rayDelay: $e")
  ...
  private fun showNotification() {
    ...
    stopV2RayIntent.`package` = packageName
  ...
  private fun updateNotification(contentText: String?, proxyTraffic: Long, directTraffic: Long) {
    if (mBuilder != null) {
      if (proxyTraffic < NOTIFICATION_ICON_THRESHOLD && directTraffic < NOTIFICATION_ICON_THRESHOLD) {
        mBuilder?.setSmallIcon(R.drawable.ic_vpn_service) // ic_stat_name
      } else if (proxyTraffic > directTraffic) {
        mBuilder?.setSmallIcon(R.drawable.ic_vpn_service) // ic_stat_proxy
      } else {
        mBuilder?.setSmallIcon(R.drawable.ic_vpn_service) // ic_stat_direct
      }
